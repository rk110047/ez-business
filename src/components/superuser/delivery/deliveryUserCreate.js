...
export default class DeliveryUserCreate extends React.Component{
	state = {
		username:"",
		email:"",
		password1:"",
	...
	registerUser = async() =>{
		...
		var self=this;
		let res = await axios.post("http://100.25.15.160/auth/special/register/",{
		...
		if (res.data.token) {
				await AsyncStorage.setItem('deliveryUserToken',"Token "+res.data.token)
				this.props.navigation.navigate('createDeliveryProfile')
				} else {
	...
	render(){
		...
		<View>
			<View style={{backgroundColor:"black",flexDirection:"row",alignItems:"center"}}>
						...
						<Text style={{fontSize:25,color:"#fff"}}> Create Delivery User</Text>
				</View>
