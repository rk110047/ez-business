...
export default class CollectorProfileRUD extends React.Component{
	...
	async getObject(){
		...
		let res  		= 	await axios.get("http://100.25.15.160/collector/profile/RUD/"+data.id)
	...
	profileCreateHandler = async() => {
     this.setState({loading:true})
      ...
      console.log(newData)
      let response = await fetch("http://100.25.15.160/collector/profile/RUD/"+this.state.data.id+"/", {
         method: 'PUT',
         headers: {
    ...
    deleteDeliveryProfileHandler = async() =>{
      	this.setState({loading:true})
      	await axios.delete("http://100.25.15.160/collector/profile/RUD/"+this.state.data.id +"/")
