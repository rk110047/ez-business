...
export default class CollectorUsers extends Component{
	...
	shopsListHandler = async() =>{
		let res 	=	await	axios.get("http://100.25.15.160/collector/profile/create/")
	...
	shopListMapHandller =()=>{
		return this.state.shops.map((i,key)=>{
			return (
				<TouchableOpacity key1={key} onPress={()=>{this.props.navigation.navigate('collectorUserProfileRUD',{data:i})}}
	...
	render(){
		...
		<View style={{flex:1}}>
			<View style={{backgroundColor:"black",flexDirection:"row",alignItems:"center"}}>
						...
						<Text style={{fontSize:25,color:"#fff"}}>Collectors</Text>
				</View>
